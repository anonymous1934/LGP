...
def base_parser():
    ...
    group.add_argument(
        ...
        nargs="?",
        type=float,
        help="Drop path rate (alternative to -drop_path_rate).",
    )
    group.add_argument(
    ...
    group.add_argument(
        ...
        nargs="?",
        type=float,
        help="Weight decay factor (alternative to -weight_decay).",
    )
    ...
    group.add_argument(
        ...
        nargs="?",
        type=float,
        help="Label smoothing factor (alternative to -label_smoothing).",
    )
    group.add_argument(
        "-sched",
        nargs="?",
    ...
    # Wavelet embedding parameters (for ViT_LGP)
    group = parser.add_argument_group("Wavelet embedding (ViT_LGP)")
    group.add_argument(
        "-use_wavelet_embedding",
        action="store_true",
        help="Enable wavelet embedding for local-global processing.",
    )
    group.add_argument(
        "-wavelet_name",
        nargs="?",
        default="haar",
        type=str,
        help="Wavelet function name (e.g., haar, db4, bior2.2).",
    )
    group.add_argument(
        "-wavelet_levels",
        nargs="?",
    ...
    group.add_argument(
        "-wavelet_fusion",
        action="store_true",
        help="Enable adaptive fusion network for local-global features.",
    )
    group.add_argument(
        "-channel_dropout",
        nargs="?",
        type=float,
        default=default_kwargs["channel_dropout"],
        help="Channel dropout rate for multi-frequency channels in wavelet embedding.",
    )
    group.add_argument(
        "-token_dropout",
        nargs="?",
        type=float,
        default=default_kwargs["token_dropout"],
        help="Token dropout rate for patch tokens in wavelet embedding.",
    )
    group.add_argument(
        "-fusion_drop_path",
        nargs="?",
        type=float,
        default=default_kwargs["fusion_drop_path"],
        help="Drop path rate for the adaptive fusion network.",
    )

    # AFNO specific parameters
    group = parser.add_argument_group("AFNO specific parameters")
    group.add_argument(
        "--sparsity-threshold",
        nargs="?",
        type=float,
        default=0.01,
        help="Sparsity threshold for AFNO models.",
    )
    group.add_argument(
        ...
        type=float,
        default=1.0,
        help="Hard thresholding fraction for AFNO models.",
    )
    group.add_argument(
        "--ema",
        nargs="?",
    ...
    group.add_argument(
        ...
        type=int,
        default=0,
        help="Local rank for distributed training (automatically set by torchrun/launch)",
    )
    group.add_argument(
        "--rank",
        type=int,
...
def parse_args(args=None):
    ...
    if "drop_path" in parsed_args and parsed_args["drop_path"] is not None:
        parsed_args["drop_path_rate"] = parsed_args["drop_path"]
    
    if "smoothing" in parsed_args and parsed_args["smoothing"] is not None:
        parsed_args["label_smoothing"] = parsed_args["smoothing"]
    
    if "weight_decay" in parsed_args and parsed_args["weight_decay"] is not None:
