...
# 添加对ViT_LGP的导入
from architectures.vit_lgp import ViT_LGP
import importlib
import os
...
def prepare_model(model_str, args):
    ...
    If the name is of the format ViT_LGP-<size>/<patch_size>, use a *ViT_LGP*.
    ...
    # 添加ViT_LGP支持
    if model_str.startswith("ViT_LGP"):
        # Format: ViT_LGP-{Ti,S,B,L}/<patch_size>[_<image_res>]
        ...
        # 获取小波相关参数
        wavelet_kwargs = {}
        if hasattr(args, "use_wavelet_embedding") and args.use_wavelet_embedding:
            wavelet_kwargs["wavelet_name"] = getattr(args, "wavelet_name", "haar")
            wavelet_kwargs["wavelet_levels"] = getattr(args, "wavelet_levels", 4)
        
        kwargs.update(wavelet_kwargs)
        
        model = ViT_LGP(
            patch_size=int(patch_size),
            in_chans=3,
            **vit_sizes[model_size],
        ...
        # 确保 vit_sizes 中的关键参数不被 kwargs 覆盖
        vit_config = vit_sizes[model_size].copy()
        safe_kwargs = {k: v for k, v in kwargs.items() if k not in vit_config}
        
        model = TimmViT(
            patch_size=int(patch_size),
            in_chans=3,
            **vit_config,  # 使用复制的配置，确保不被覆盖
            norm_layer=partial(nn.LayerNorm, eps=1e-6),
            **safe_kwargs,  # 只传递不冲突的参数
        )

    else:
        # 支持新的LGP模型名称格式: vit_{size}_patch16_lgp
        if model_str.endswith("_lgp"):
            # 这些模型已经通过timm注册，直接创建
