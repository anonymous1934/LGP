...
def prepare_dataset(dataset_name, args, transform=None, train=True, rank=None):
    ...
    if dataset_name == "cifar10":
        ...
        # 检查数据集格式：msgpack vs 标准文件夹格式
        msgpack_path = f"{args.dataset_root}imagenet/msgpack/{'train' if train else 'val'}.msgpack"
        folder_path = os.path.join(args.dataset_root, 'train' if train else 'val')
        ...
        if os.path.exists(msgpack_path):
            ...
            collate = collate_imnet
            
        elif os.path.exists(folder_path):
            # 使用标准ImageNet文件夹格式
            print(f"Using ImageNet folder format from: {folder_path}")
            dataset = ImageFolder(
                root=folder_path,
                transform=tv_transforms.Compose(transform)
            )
            collate = None  # 使用默认的collate函数
            
        else:
            raise ValueError(f"Neither msgpack format ({msgpack_path}) nor folder format ({folder_path}) found!")
        ...
        if "21k" in dataset_name:
            ...
            if os.path.exists(folder_path):
                n_classes = len([d for d in os.listdir(folder_path) 
                               if os.path.isdir(os.path.join(folder_path, d))])
                print(f"Detected {n_classes} classes in dataset")
            else:
                n_classes = 100  # 默认ImageNet100
